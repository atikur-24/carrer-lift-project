...
const AppliedDetails = ({ apply }) => {
    ...
    return (
            <div className='flex items-center p-7 mb-6 border border-inherit rounded-md'>
                <div className='bg-[#F4F4F4] h-60 w-60 flex items-center justify-center'>
                    <img src={logo} alt="logo" />
                </div>
                <div className='ml-8 space-y-4'>
                    <h2 className='text-2xl font-extrabold text-[#474747]'>{job_title}</h2>
                    <p className='text-2xl font-semibold text-[#757575]'>{brand_name}</p>
                ...
                <div className='ml-auto'>
                    <button onClick={ () => navigate(`/details/${id}`) } className='btn w-40 h-16'>View Details</button>
                </div>
            </div>
    );
};
